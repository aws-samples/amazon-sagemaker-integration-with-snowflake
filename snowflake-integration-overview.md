...
functions](https://docs.snowflake.com/en/sql-reference/external-functions-introduction.html)
and [request
translators](https://docs.snowflake.com/en/LIMITEDACCESS/external-functions-serializers.html)
...
1.  When a supported AWS_AUTOPILOT SQL command is executed, the UI
     ...
     any request translators and response translators associated with the
     external function.
...
-   **S3 bucket** to store the training data and model artifacts created
     by Autopilot. See [S3
     documentation](https://aws.amazon.com/s3/getting-started/)
     to learn more about the service.
...
-   **IAM roles** to access the resources and set up trust relationships
     ...
     roles](https://docs.snowflake.com/en/sql-reference/external-functions-creating-aws-common-api-integration-proxy-link.html).
...
-   **Storage Integration** required to copy data from a Snowflake table
     ...
     on [Storage
     Integrations](https://docs.snowflake.com/en/sql-reference/sql/create-storage-integration.html)
     to learn more.
...
-   **API Integration** required by the Snowflake external functions to
     ...
     Integrations](https://docs.snowflake.com/en/sql-reference/sql/create-api-integration.html)
     to learn more.

-   **External functions and associated request translators and response
     translators** that correspond to various SageMaker calls. See
     Snowflake's documentation on [External
     Functions](https://docs.snowflake.com/en/sql-reference/external-functions-introduction.html)
     and [Request
     Translators](https://docs.snowflake.com/en/LIMITEDACCESS/external-functions-serializers.html)
     to learn more.
...
at [https://aws.amazon.com](https://aws.amazon.com). Your AWS
...
console](https://console.aws.amazon.com/servicequotas/home?region=us-east-2#!/).
...
quotas](https://docs.aws.amazon.com/general/latest/gr/sagemaker.html#limits_sagemaker)
for your AWS account. If you encounter error messages that you\'ve
exceeded your quota, use [AWS
Support](https://console.aws.amazon.com/support/) to request a
...
functions](https://docs.aws.amazon.com/IAM/latest/UserGuide/access_policies_job-functions.html).
...
[roles](https://docs.snowflake.com/en/user-guide/security-access-control-overview.html#roles)
and
[privileges](https://docs.snowflake.com/en/user-guide/security-access-control-overview.html#privileges)
...
6.  **snowflakeDatabaseName** - Enter the name of the Snowflake Database
     in which to create the external functions and request translators.

7.  **snowflakeSchemaName** - Enter the name of the Snowflake Database
     Schema in which to create the external functions and request
     translators.
...
## Working with SageMaker APIs from Snowflake
...
-   The [Problem
     ...
     and [Objective
     metric](https://docs.aws.amazon.com/sagemaker/latest/APIReference/API_AutoMLJobObjective.html#sagemaker-Type-AutoMLJobObjective-MetricName)
     are auto inferred.
...
-   Once the best model is found Autopilot transparently deploys the
    ...
    -   If you would like to redeploy the model after it has been
         deleted, use the
         [AWS_AUTOPILOT_CREATE_ENDPOINT](#create-endpoint)
...
Amazon SageMaker Clarify provides machine learning developers with
greater visibility into their training data and models so they can
identify and limit bias and explain predictions. During the model
training process, SageMaker Autopilot automatically creates a notebook
(and PDF report) that displays the 10 features with the greatest feature
attribution. The notebook is stored in:

`<s3 bucket>/output/<model>/documentation/explainability/output/<training run>`
...
-   Delete any Sagemaker endpoints that were provisioned while using the
    ...
    -   Using the [Delete Endpoint](#delete-endpoint) SQL command
         from Snowflake or
    ...
    -   By opening the Amazon SageMaker console at
         ...
         deletes the ML compute instances that support it.
...
-   Delete any Sagemaker endpoint configurations that were provisioned
    ...
    -   Using the [Delete Endpoint
         Config](#delete-endpoint-config) SQL command from
         Snowflake or
...
-   Delete any Sagemaker Autopilot Models that were created. You can do
    ...
    -   By opening the Amazon SageMaker console at
        ...
        -   Choose the model that you created. Choose Actions, and then
             choose Delete.
...
-   Log in to the AWS console and navigate to CloudFormation service.
    ...
    -   To delete the S3 bucket, you need to navigate to the S3 service
         ...
         [Deleting a
         bucket](https://docs.aws.amazon.com/AmazonS3/latest/userguide/delete-bucket.html).
